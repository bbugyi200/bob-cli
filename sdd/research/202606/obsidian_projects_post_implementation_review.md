...
status: research
topic: Post-implementation review of Bob vault projects support + domain best practices
---
...
1. **Placeholder mismatch**: the CLI only recognizes `<short_project_completion_criteria_goes_here>`
   ...
   `_templates/new_project.md` emits `(REPLACE WITH PROJECT COMPLETION CRITERIA)`. Projects created via
   `Ctrl+Shift+N` with an unedited placeholder are never flagged. `project.md` documents the CLI's string.
...
3. **Counting scope diverges**: the plugin counts only the `## Tasks` section (`taskSectionLines()`), so the `^prj`
   task (above the title) is excluded from `task_count`; CLI `list`'s open count scans the whole body. "Open tasks"
   in `projects.base` and `bob projects list` are different numbers by construction.
4. **Template/heading drift**: `_templates/new_project.md` creates `## Project Notes`; `project.md` and the epic's
   design contract specify `## Project Support`.
5. **Missing `[created::]`**: the template's `^prj` task has no `[created::]` stamp even though `project.md`
...
6. **Parent-type validation asymmetry**: the hotkey plugin enforces parent type ∈ {area, project}
...
7. **Tag-boundary regex duplication**: `#task` boundary detection is independently implemented in JS regex
...
- **Staleness is a second, distinct failure mode**: a project can have open tasks yet be stalled because nothing
  ...
  sanctioned stall detector ([checklist PDF](https://gettingthingsdone.com/wp-content/uploads/2014/10/Weekly_Review_Checklist.pdf)).
...
- **Outcome statements**: phrase project completion as a binary-checkable outcome
  ...
  exactly this pattern; the placeholder-warning fix (above) is what enforces it.
...
- **Tasks-plugin line grammar**: Tasks parses task lines backwards from the end and stops at the first unrecognized
  ...
  ([issue 3267](https://github.com/obsidian-tasks-group/obsidian-tasks/issues/3267)).
...
1. **Fix the placeholder mismatch**: make the CLI recognize the template's
   `(REPLACE WITH PROJECT COMPLETION CRITERIA)` (or better, both strings), and align `project.md` +
   `_templates/new_project.md` on one canonical placeholder. Until then the placeholder warning never fires for
   hotkey-created projects.
2. **Rename `## Project Notes` → `## Project Support`** in `_templates/new_project.md` (matches `project.md`, the
   design contract, and GTD's "project support" term).
3. **Add `[created::<% tp.file.creation_date("YYYY-MM-DD") %>]`** to the template's `^prj` task.
4. **Unify open-status semantics**: pick one rule — recommend the CLI's "not `x`/`X`/`-` ⇒ open" (robust to new
   custom statuses) — and apply it in the plugin. Document the intended scope difference (plugin counts `## Tasks`
   only; CLI scans body) or eliminate it; at minimum rename CLI column headers so the two "open counts" aren't
   conflated.
5. **CLI parent validation**: `bob projects list` should warn when a project's `parent` is missing or resolves to a
   non-area/non-project note (parity with the hotkey's `PROJECT_PARENT_TYPE_BASENAMES` check).

### P1 — GTD gaps (highest behavioral value)

6. **Add a `someday` (or `hold`) status**: exempt from `^prj`-surfacing/stall logic, excluded from Active views,
   ...
   dashboard).
7. **Three-way stall reporting in `bob projects list`/`sync`**: red = `wip` with no open task, no waiting item, no
   ...
   `projects.base` via `file.mtime < now() - "2w"` and `date.relative()` — native, no new automation.
8. **Waiting-for dates**: adopt `[waiting:: YYYY-MM-DD]` (date delegated) on waiting tasks and/or a `waiting_since`
...
9. **Pin the dash embed to a named view**: `![[projects.base#🚀 Active & Waiting]]` instead of relying on view
   order.
10. **Per-area project lists**: one reusable embedded base (or `base` code block) in area notes filtered by
    `note.parent == this.file.asLink()` — gives each area its own live project index for reviews.
11. **Optional sub-project rollup column** via `parent.asFile().properties.status` lookups (mind the
...
12. **Atomic CAS writes in `bob projects sync`**: dot-prefixed same-dir temp file + fsync + re-stat (abort if
...
13. **Preserve Tasks line grammar invariants** when editing `^prj` lines: block ID last; never duplicate an
    identical task line; keep the existing dedupe/normalize pass for the `🧩 **Sub-projects:**` marker.
14. **Share the task-grammar definition**: extract the `#task` boundary + status rules into one spec (a small
    fixture-based conformance test run against both the JS plugin and Rust CLI would prevent silent drift).
15. **Sync settings on the cron machine**: prefer "Create conflict file" over auto-merge; schedule syncs outside
...
`~/bob/.obsidian/plugins/bob-navigation-hotkeys/main.js`, `src/native/projects.rs`.
