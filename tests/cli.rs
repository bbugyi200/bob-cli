...
#[test]
fn highlights_ref_scan_degrades_missing_plain_markdown_image_selection() {
    let temp =
        TempDir::new("bob-cli-highlights-ref-missing-plain-image-scan");
    let vault = temp.path().join("vault");
    let pdf = vault.join("lib/docs/gastown_readme.pdf");
    let sidecar = pdf.with_extension("md");
    let note = vault.join("ref/docs/gastown_readme.md");
    let other_pdf = vault.join("lib/books/other.pdf");
    let other_note = vault.join("ref/books/other.md");

    write_highlights_pdf(
        &pdf,
        "- status: wip\n- parent: obsidian\n- title: Gastown README\n",
    );
    write_file(
        &sidecar,
        "\
...
Comment: Missing plain Markdown image export.

---

> Town highlight still syncs.
",
    );
    write_highlights_pdf(
        &other_pdf,
...
",
    ...
    let output = bob_command()
        ...
        .env("BOB_DIR", &vault)
        .output()
        .expect("dry-run scan with missing plain markdown image");

    assert_success(&output);
    let dry_run = stdout(&output);
    assert!(dry_run.contains(path_str(&note)), "{dry_run}");
    assert!(dry_run.contains("notes_create: 2"), "{dry_run}");
    assert!(dry_run.contains("images_unresolved: 1"), "{dry_run}");
    assert!(
        dry_run.contains("image_warning: image not synced: assets/area-selection.png - image asset not found: assets/area-selection.png"),
        "{dry_run}"
    );
    assert!(dry_run.contains("plan_failures: 0"), "{dry_run}");
    assert!(!dry_run.contains("plan_error:"), "{dry_run}");
    assert!(!note.exists(), "dry-run must not create note");
    assert!(!other_note.exists(), "dry-run must not create other note");
    ...
    let output = bob_command()
        ...
        .env("BOB_DIR", &vault)
        .output()
        .expect("write scan with missing plain markdown image");

    assert_success(&output);
    let written = stdout(&output);
    assert!(written.contains("notes_created: 2"), "{written}");
    assert!(written.contains("images_unresolved: 1"), "{written}");
    assert!(written.contains("plan_failures: 0"), "{written}");
    assert!(!written.contains("partial-failure"), "{written}");
    let contents =
        fs::read_to_string(&note).expect("read degraded image note");
    assert!(contents.contains("highlights_count: 2\n"), "{contents}");
    assert!(
        contents.contains("> [!warning] Image not synced"),
        "{contents}"
    );
    assert!(
        contents.contains(
            "Reason: image asset not found: assets/area-selection.png."
        ),
        "{contents}"
    );
    assert!(
        contents.contains("> [!quote] Town highlight still syncs."),
        "{contents}"
    );
    assert!(other_note.exists(), "scan should still write other PDFs");
    ...
    let output = bob_command()
        ...
        .env("BOB_DIR", &vault)
        .output()
        .expect("repeat scan with missing plain markdown image");

    assert_success(&output);
    let repeated = stdout(&output);
    assert!(repeated.contains("notes_unchanged: 2"), "{repeated}");
    assert!(repeated.contains("images_unresolved: 1"), "{repeated}");
    assert!(repeated.contains("writes: none"), "{repeated}");
    assert!(!repeated.contains("partial-failure"), "{repeated}");
}
